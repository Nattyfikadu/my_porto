  ...
  return (
    <div ref={ref} className={styles.wrap}>
      <button
        ...
        onClick={() => setOpen(v => !v)}
        aria-label={`Contact ${profile.shortName}`}
        aria-expanded={open}
      >
      ...
      <div className={`${styles.dropdown} ${open ? styles.dropdownOpen : ''}`}>
        {contacts.map((c, i) => (
          <a
            ...
            style={{ '--i': i, animationDelay: open ? `${i * 50}ms` : '0ms' } as React.CSSProperties}
            onClick={() => setOpen(false)}
          >
            <span className={styles.iconCircle} style={{ color: c.color }}>
              {c.icon}
            </span>
